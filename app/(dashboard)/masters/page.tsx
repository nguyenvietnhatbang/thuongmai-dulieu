...
export default async function MastersPage() {
  ...
  if (!currentUser) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <p className="text-muted-foreground">Vui lòng đăng nhập để xem danh mục cơ sở.</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Tài khoản của bạn (<span className="font-mono text-xs">{currentUser.email}</span>) không được cấp quyền xem danh mục cơ sở.
        </p>
        <Link
          href="/"
  ...
  return <MastersClient currentUser={currentUser} />;
}
