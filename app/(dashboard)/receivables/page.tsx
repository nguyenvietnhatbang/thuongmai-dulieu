...
export default async function ReceivablesPage() {
  ...
  if (!currentUser) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <p className="text-muted-foreground">Vui lòng đăng nhập để xem thông tin công nợ.</p>
      </div>
    );
  }
  ...
  const canView = currentUser.roles.includes('system_management') ||
    await hasPermission('receivables.view.team');
    ...
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Tài khoản của bạn (<span className="font-mono text-xs">{currentUser.email}</span>) không được cấp quyền xem danh sách công nợ phải thu.
        </p>
        <Link
          href="/"
  ...
  return <ReceivablesClient currentUser={currentUser} />;
}
