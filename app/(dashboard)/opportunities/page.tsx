...
export default async function OpportunitiesPage() {
  ...
  if (!currentUser) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <p className="text-muted-foreground">Vui lòng đăng nhập để xem danh sách cơ hội.</p>
      </div>
    );
  }

  const canView = currentUser.roles.includes('system_management') ||
    currentUser.permissions.includes('opportunities.view.own') ||
    currentUser.permissions.includes('quotes.view.team') || // business managers/sales reps can view
    ...
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Tài khoản của bạn (<span className="font-mono text-xs">{currentUser.email}</span>) không được cấp quyền xem cơ hội bán hàng.
        </p>
        <Link
          href="/"
  ...
  return <OpportunitiesClient currentUser={currentUser} />;
}
