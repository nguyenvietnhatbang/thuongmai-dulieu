...
export default async function ContractDetailPage({
  ...
  if (!currentUser) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <p className="text-muted-foreground">Vui lòng đăng nhập để xem hồ sơ hợp đồng.</p>
      </div>
    );
  }
  ...
  const canView = currentUser.roles.includes('system_management') ||
    ...
    currentUser.permissions.includes('receivables.view.team') ||
    ...
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Tài khoản của bạn (<span className="font-mono text-xs">{currentUser.email}</span>) không được cấp quyền xem hồ sơ hợp đồng.
        </p>
        <Link
          href="/contracts"
  ...
  return <ContractDetailClient contractId={id} currentUser={currentUser} />;
}
