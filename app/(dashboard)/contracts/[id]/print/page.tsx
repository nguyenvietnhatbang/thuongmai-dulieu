      ...
      <div className="max-w-4xl mx-auto bg-white p-8 border border-slate-200 rounded-lg shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-2 gap-8 text-xs">
          <div className="space-y-2">
            <h3 className="font-bold text-slate-500 uppercase tracking-wider text-[10px]">Bên A — {companySettings.navName} (Bên cung cấp dịch vụ)</h3>
            <p className="font-extrabold text-slate-800 text-sm">{companySettings.companyName}</p>
            {companySettings.taxCode && <p className="text-slate-600">Mã số thuế: {companySettings.taxCode}</p>}
            {companySettings.address && <p className="text-slate-600">Địa chỉ: {companySettings.address}</p>}
            <p className="text-slate-600">
              Đại diện ký: {companySettings.representativeName || contract.ownerName || '...................................'}
              {companySettings.representativeTitle ? ` - ${companySettings.representativeTitle}` : ''}
            </p>
          </div>
          <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-8 text-xs text-center pt-8 border-t border-slate-200 mt-6">
          ...
          <div>
            <p className="font-bold text-slate-700">ĐẠI DIỆN BÊN A ({companySettings.navName})</p>
            <p className="text-[10px] text-slate-400 mt-1">(Ký, ghi rõ họ tên và đóng dấu)</p>
            <div className="h-24" />
          </div>
        </div>
