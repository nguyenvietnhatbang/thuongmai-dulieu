  ...
  const renderContent = () => (
    <div className="space-y-6 text-sm">
      <div className="grid grid-cols-2 gap-y-4 gap-x-2 border border-border p-4 rounded-xl bg-slate-50/50">
        <div>
          <p className="text-[10px] text-muted-foreground font-bold uppercase">Mã kho</p>
          <p className="font-mono font-bold text-primary mt-0.5">{warehouse.code}</p>
        </div>
        <div>
          <p className="text-[10px] text-muted-foreground font-bold uppercase">Tên kho hàng</p>
          <p className="font-bold text-foreground mt-0.5">{warehouse.name}</p>
        </div>
        <div className="col-span-2">
          <p className="text-[10px] text-muted-foreground font-bold uppercase">Địa chỉ kho</p>
          <p className="font-semibold text-foreground mt-0.5">{warehouse.address || '-'}</p>
        </div>
      </div>
      ...
      subtitle={
        <div className="flex items-center gap-2 mt-1">
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${
            warehouse.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-100 text-slate-600 border-slate-200'
          }`}>
