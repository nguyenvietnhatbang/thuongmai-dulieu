...
export default async function ProjectDetailPage({
  ...
  if (!currentUser) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <p className="text-muted-foreground">Vui lòng đăng nhập để xem hồ sơ dự án.</p>
      </div>
    );
  }
  ...
  const canView = currentUser.roles.includes('system_management') ||
    ...
    currentUser.permissions.includes('tasks.view.own') ||
    ...
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Tài khoản của bạn (<span className="font-mono text-xs">{currentUser.email}</span>) không được cấp quyền xem hồ sơ dự án.
        </p>
        <Link
          href="/projects"
