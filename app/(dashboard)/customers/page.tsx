...
export default async function CustomersPage() {
  ...
  if (!currentUser) {
    return (
      <div className="flex h-[50vh] items-center justify-center">
        <p className="text-muted-foreground">Vui lòng đăng nhập để xem thông tin khách hàng.</p>
      </div>
    );
  }

  // Double check basic view permission at the page boundary
  const canView = currentUser.roles.includes('system_management') ||
    currentUser.permissions.includes('customers.view.own') ||
    currentUser.permissions.includes('customers.view.team') ||
    currentUser.permissions.includes('customers.view.all');
    ...
    return (
      <div className="max-w-md mx-auto mt-20 glass-panel rounded-2xl p-8 text-center space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Tài khoản của bạn (<span className="font-mono text-xs">{currentUser.email}</span>) không được cấp quyền xem danh sách khách hàng.
        </p>
        <Link
          href="/"
  ...
  return <CustomersClient currentUser={currentUser} />;
}
