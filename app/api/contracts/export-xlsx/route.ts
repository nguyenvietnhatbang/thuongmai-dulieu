...
export async function GET(request: Request) {
  try {
    ...
    const sort = parseSort(searchParams, contractSorts, 'createdAt');
    ...
    worksheet.getCell('A1').value = 'DANH SÁCH HỢP ĐỒNG';
    ...
    result.data.forEach((contract, index) => {
      ...
      const row = worksheet.addRow([
        ...
        contract.projectCreated ? 'Có' : 'Chưa',
    ...
    const totalRow = worksheet.addRow(['', 'TỔNG', '', '', '', '', totalValue, '', '', '', '']);
    ...
    console.error('Contracts Excel export error:', error);
